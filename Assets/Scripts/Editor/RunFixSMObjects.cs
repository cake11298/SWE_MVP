...
namespace BarSimulator.Editor
{
    ...
    /// 自动运行修复 SM 物件的脚本
    /// </summary>
    public static class RunFixSMObjects
    {
        [MenuItem("Tools/Auto Fix All SM Objects Now")]
        public static void Execute()
        {
            Debug.Log("[RunFixSMObjects] Starting to fix all SM objects...");
            ...
            Debug.Log($"[RunFixSMObjects] Complete! Found {totalSMObjects} SM objects, fixed {fixedCount} objects");
            EditorUtility.DisplayDialog("Complete", 
                $"Found {totalSMObjects} SM objects\nFixed {fixedCount} objects\n\nAll SM objects are now ready for interaction!", 
                "OK");
        }
        ...
        private static bool FixObject(GameObject obj)
        {
            ...
            if (rb == null)
            {
                ...
                modified = true;
                Debug.Log($"[RunFixSMObjects] Added Rigidbody to {obj.name}");
            }
            ...
            if (collider == null)
            {
                ...
                if (meshFilters.Length > 0)
                {
                    ...
                    if (boundsInitialized)
                    {
                        ...
                        modified = true;
                        Debug.Log($"[RunFixSMObjects] Added BoxCollider to {obj.name} (size: {boxCollider.size})");
                    }
                    else
                    {
                        // 添加默认 BoxCollider
            ...
            if (staticProp == null)
            {
                ...
                modified = true;
                Debug.Log($"[RunFixSMObjects] Added StaticProp to {obj.name}");
            }
            ...
            if (interactableItem == null)
            {
                ...
                modified = true;
                Debug.Log($"[RunFixSMObjects] Added InteractableItem to {obj.name}");
            }
        ...
        private static void AddDefaultCollider(GameObject obj)
        {
            ...
            Debug.Log($"[RunFixSMObjects] Added default BoxCollider to {obj.name}");
        }
